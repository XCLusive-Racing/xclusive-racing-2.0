        ...
        <div class="col-12 col-lg-8">
            ...
            <div class="admin-card mb-4">
                <div class="px-4 pt-4 pb-3">
                    ...
                    {{-- Format info card (shown after format pick) --}}
...
(function () {
    ...
    fmtEl.addEventListener('change', () => {
        ...
        const id   = fmtEl.value;
        const fmt  = (formats[game] || []).find(f => String(f.id) === id);
    ...
    // Toggle panels
    [['ce-sr-toggle', 'ce-sr-panel'], ['ce-minrating-toggle', 'ce-minrating-panel'], ['ce-maxrating-toggle', 'ce-maxrating-panel']].forEach(([toggleId, panelId]) => {
        const toggle = document.getElementById(toggleId);
        const panel  = document.getElementById(panelId);
        toggle.addEventListener('change', () => { panel.style.display = toggle.checked ? '' : 'none'; });
    });

    // Init on load (old values after validation error)
    if (oldGame) {
